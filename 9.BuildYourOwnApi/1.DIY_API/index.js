...
app.get("/jokes/:id", (req, res) => {
  ...
  const findJoke = jokes.find((item) => item.id === id);
  res.json(findJoke);
})
...
app.get("/filter", (req, res) => {
  ...
  const findJoke = jokes.find((joke) => joke.jokeType === type);
  res.json(findJoke);
})
...
app.delete("jokes/:id", (req, res) => {
  ...
  if(index > -1) {
    ...
    res
       .status(400)
       .json({error : `joke with id ${id} not found, so no joke will be delted`});
  }
})
